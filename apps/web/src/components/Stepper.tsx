...
const Check: React.FC<CheckProps> = ({ number, step }) => {
  return (
    <div
      className={`
      h-6 w-6 rounded-full border-2 border-bc-blue-nav
      text-white
      flex flex-row items-center justify-center
      ${step >= number && 'bg-bc-blue-nav'}`}
    >
      {(() => {
        switch (true) {
          case number === step:
  ...
  return (
    <div className='flex flex-col items-start'>
      <div aria-hidden className='flex justify-center items-center'>
        ...
        {!isLast ? <div className='grow md:w-32 w-4 mx-2 border-t-2 border-bc-blue-nav' /> : null}
      </div>
